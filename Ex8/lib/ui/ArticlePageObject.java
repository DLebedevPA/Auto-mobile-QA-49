...
public class ArticlePageObject extends MainPageObject
{
    ...
    {
        this.waitForElementAndClick(
                By.xpath(OPTIONS_BUTTON), "Cannot find button to open article options", 15
        );

        Thread.sleep(1000);

        this.waitForElementAndClick(
                By.xpath(OPTIONS_ADD_TO_MY_LIST), "Cannot find find option to add article to reading list", 5
        );

        this.waitForElementAndClick(
                By.id(ADD_TO_MY_LIST_OVERLAY), "Cannot find 'Got it' tip overlay", 5
        );

        this.waitForElementAndClear(
                By.id(MY_LIST_NAME_INPUT), "Cannot find input to set name of articles folder", 5
        );
